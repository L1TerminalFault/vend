...
export default function TransactionsPage() {
  ...
  // FIX: Added a DOM node ref to catch clicks outside this area
  const filterDropdownRef = useRef<HTMLDivElement>(null);

  // FIX: Explicit click-away event handling for robust cross-browser reliability
  useEffect(() => {
  ...
  return (
    <div className="w-full h-full flex flex-col gap-6 z-0 p-6 px-4 md:px-8 overflow-y-auto mb-[100px] scrollbar-hidden">
      <div ref={headerRef} className="flex flex-col z-1 sm:flex-row w-full gap-4 mb-2 justify-between sm:items-end">
        <div className="flex flex-col gap-2 w-full">
          <h2 className="text-3xl font-extrabold tracking-tight flex items-center gap-3">
            <FiFileText className="text-theme-text" /> Activity Log
          </h2>
        ...
        <div className="flex items-center gap-3 w-full justify-end">
          {/* FIX: Bound the target layout element using a relative ref box wrapper */}
      ...
      <div className="flex flex-wrap gap-2 p-1 bg-theme-card border border-theme-border/50 rounded-full w-fit relative z-0">
        {tabs.map((tab) => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key)}
          ...
          >
            {tab.icon && <tab.icon />}
            {tab.label}
            <span className="text-xs max-sm:hidden -z-10 opacity-80">({tab.count})</span>
          </button>
        ))}
      </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col items-end shrink-0">
                  ...
                  >
                    {isRefill ? '' : `$${getTransactionTotal(item as TransactionType).toFixed(2)}`}
                  </span>
                  <span className="text-[10px] text-theme-text/40 uppercase">
                    {new Date(item.createdAt!).toLocaleString()}
                  </span>
