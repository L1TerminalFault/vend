...
export default function HomePage() {
  ...
  // FIX: Outside-click anchor ref for the machine selection dropdown
  const machinePickerRef = useRef<HTMLDivElement>(null);

  // FIX: Robust global event tracking to dismiss the machine picker picker cleanly anywhere on screen
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (machinePickerRef.current && !machinePickerRef.current.contains(event.target as Node)) {
  ...
  const renderAdminDash = () => (
    <>
      ...
      <div ref={headerRef} className="flex flex-col sm:flex-row w-full justify-between items-start sm:items-center gap-4 border-b border-theme-border/50 pb-6 pt-2">
        ...
        <div className="flex items-center gap-3 w-full justify-end">
          {/* FIX: Bound layout wrapper with dropdown listener ref box */}
